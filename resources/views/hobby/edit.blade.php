    ...
    @section('content')
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white shadow-md rounded-lg p-5 w-full max-w-lg">
                <h2 class="text-2xl font-semibold text-center text-gray-900">Edit Hobby</h2>
                <form action="{{ route('hobby.update', $hobby->id) }}" method="POST" role="form" enctype="multipart/form-data" class="mt-4">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="nama_hobby" class="block text-lg font-medium text-gray-700">Nama Hobby</label>
                        <input type="text" name="nama_hobby" id="nama_hobby" required
