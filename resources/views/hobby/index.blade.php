    ...
    @section('content')
        ...
        <div class="card prose container px-6 py-3 mt-4">
            ...
            <table class="border-collapse border border-slate-500 ...">
                ...
                <tbody>
                    @php $no = ($hobby->currentPage() - 1) * $hobby->perPage() + 1; @endphp
                    @foreach ($hobby as $data)
                        <tr>
                            <th class="text-center border">{{ $no++ }}</th>
                            <td class="text-center border">{{ $data->nama_hobby }}</td>
                            <form action="#" method="POST">
                                @csrf
                                @method('DELETE')
                                <td class="border text-center">
                                    <a href="{{ route('hobby.edit', $data->id) }}"
                                        class="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded mx-2 no-underline">
                                        Edit
                                    </a>
                                    <a href="{{ route('hobby.destroy', $data->id) }}" type="submit"
